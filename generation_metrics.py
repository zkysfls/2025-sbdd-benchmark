...
def process_all_folders(root_directory):
    ...
    uniqueness_eval = Evaluator(name = 'Uniqueness')
    validity_eval = Evaluator(name = 'Validity')
    ...
    for model_dir in os.listdir(root_directory + '/Generated_molecules_archeive'):
        ...
        for target_dir in os.listdir(model_path):
            ...
            csv_dir = '/data2/kai/SBDD/results/oracle2_csv_files'
            ...
            with open(csv_filename, 'w', newline='') as csvfile:
                ...
                writer.writerow(['Input smiles', 'Diversity', 'Uniqueness', 'Validity'])
                ...
                for smiles in data:
                    try:
                        diversity = diversity_eval(smiles)
                        uniqueness = uniqueness_eval(smiles)
                        validity = validity_eval(smiles)
                        writer.writerow([smiles, diversity, uniqueness, validity])
                    except Exception as e:
